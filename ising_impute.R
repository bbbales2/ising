...
y = mclapply(mus, function(mu) { ising_kmc_deriv(x, mu, beta, kT, S, 0)$phi }, mc.cores = 24) %>% unlist() + rnorm(20) * sigma
...
dbeta = 0.0000001
(UgradU(0.25 + dbeta)$u - UgradU(0.25)$u) / dbeta
UgradU(0.25)
...
for(i in 2:length(bcs)) {
  bcs[i] = radford(UgradU, 0.1e-3, 50, bcs[i - 1])
  print(bcs[i])
